...
public class ReplyController {
	...
	//댓글 리스트 메서드(아래)
		...
		public ResponseEntity<Map<String,Object>> reply_list(@PathVariable("bno") Integer bno,@PathVariable("page") Integer page) throws Exception {
			...
			pageVO.setPage(page);//조건은 Ajax로 호출시 항상 page변수는 반드시 보내야 함.
			...
			pageVO.setTotalCount(replyCount);//전체 댓글 개수 구해서 set하는 순간이 필수 pret.next 구할때 필요
			//페이지 계산식 처리 끝
	...
	//댓글 삭제 메서드(아래) 전송방식 POST가 아니고, DELETE를 사용
	...
	public ResponseEntity<String> reply_delete(@PathVariable("bno") Integer bno,@PathVariable("rno") Integer rno) {
		...
		try {
			...
			replyDAO.updateCountReply(bno);//서브쿼리를 사용해서 tbl_board테이블 reply_count필드를 업데이트 함
		} catch (Exception e) {
			result = new ResponseEntity<String>(HttpStatus.BAD_REQUEST);
		}
		return result;
	}
	//댓글 수정 메서드(아래) 전송방식 POST(고전방식)가 아니고, PATCH(최근 방식 RestAPI)를 사용
	...
	public ResponseEntity<String> reply_write(@RequestBody ReplyVO replyVO) {
		//@RequestBody클래스는 ajax로 보내온 폼데이터를 ReplyVO 클래스에 바인딩시켜주는 애노테이션 클래스
		...
		ResponseEntity<String> result = null;
		try {//예외처리를 상위 메서드로 보내지 않는 이유 RestAPI에서 예외 메시지를 개발자가 제공하기 위해서
		...
		return result;//ResponseEntity객체클래스형 String값을 ajax로 호출한 페이지 반환.
	}
