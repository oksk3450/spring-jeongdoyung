...
public class ControllerAdviceException {
	...
	//컨트롤러 매핑이 걸린 모든메서드(jsp와 매칭)에서 공통으로 사용할 modelAttribute 생성(board_type_list)
	...
	public ModelAndView errorModelAndView(Exception ex, HttpServletRequest request) {
		...
		//여기서 사용하는 ModelAndView클래스는 jsp로 값을 보내고 + 반환값으로 model값을 보낼 jsp경로도 지정
		...
		//세션은 클라이언트가 서버에 접속시 서버에서 발생하는 고유변수값을 세션이라고 하고, prevPage세션키를 하나 생성(위)
		//세션에 저장된 prevPage키를 jsp에서 사용하게 됨.
