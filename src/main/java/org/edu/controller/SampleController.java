 ...
 * 그래서, 저장시 콘솔에 보면, Reloading Context with... is completed 재실행 되었다고 나옴.
 * 그래서, java 클래스는 수정 후 약간 기다린 후 Reloading... 메시지 후 결과 확인이 가능.
 */
@RequestMapping(value="/sample/contact",method=RequestMethod.POST)//포스트방식은 데이터를 전송하는 역할
public String contact_send() {
	//데이터 전송후에 다른 페이지 이동이 필요합니다. 새로고침=자동등록을 방지하기 위해서. 게시판 테러 방지용
...
public String weare() {
	//외부에서 /weare경로로 접근했을떄, sample/weare.jsp와 매핑시키라는 의미.
	return "sample/weare";
}
@RequestMapping(value="/sample",method=RequestMethod.GET)
public String index() { //메서드(함수)의 리턴(출력)형태지정 메서드명(매개변수) {구현내용} String index() {}
	//상단 리퀘스트매핑에서 지정한 경로로 접근을 하게되면, 아래에 있는 index()메서드를 실행해라.
	//메서드명 제일 앞에있는 public은 접근권한에 대한 키워드, public(공공-다른 클래스에서 접근가능)
	//private(내부-현재 HomeController클래스에서만 접근가능)
	//public String index() {} 자바(스프링) 메서드 기본형태
	//$(document).ready(function(){ }); j쿼리 기본형태
	//리턴반환값으로 사용한 String 클래스는 문자열을 반환
	//여기선 index.jsp와 매칭되는 문자열을 반환.
