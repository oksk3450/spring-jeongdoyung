...
<script>
//Ajax로 아이디 중복체크 후 회원등록 버튼 활성화
jQuery(document).ready(function(){
	//$대신에 jQuery를 사용하는 목적은 자바변수 $와 헷갈리지 않도록 하기위해
	//또는, PHP기반(워드프레스, 카페24쇼핑몰같은 솔루션에서 제이쿼리를 사용할때) $사용하면 작동X, jQuery로 사용.
	//단점은 $에 비해 타자치는게 힘듬.
	jQuery("#btn_join").attr("disabled","true");//초기에는 서밋버튼 비활성화.
	jQuery("#btn_join").css("opacity","0.5");//#은 엘리먼트의 id를 선택시 사용, .은 엘리먼트의 class를 선택
	jQuery("input[name='user_id']").bind("blur",function(){
		...
		jQuery.ajax({
			...
			success:function(result){
				...
				if(result=='1'){
					alert('중복 아이디가 존재 합니다. 다시 입력해 주세요.');
...
<script>
//폼서밋 전송시 인증 대기 상태를 disabled false상태로 전송(아래)
/* 아래대신 다른 방식 선택
jQuery(document).ready(function(){
	jQuery("form[name='join_form']").on("submit",function(event){
		event.preventDefault();//서밋 발생시 기본전송을 중지하고 아래내용 실행
				...
				<fieldset>
					...
					<ul class="app_list">
						...
						<li class="clear">
							...
							<div class="app_content">
							<input value="" type="text" name="user_name" class="w100p" id="user_name_lbl" placeholder="사용자명을 입력해주세요" required/>
							</div>
						</li>
						<li class="clear">
						...
						<li class="clear">
							...
							<div class="app_content">
							<input readonly value="10" type="digits" name="point" class="w100p" id="point_lbl" placeholder="포인트를 입력해 주세요" required/>
							</div>
						</li>
						<li class="clear">
