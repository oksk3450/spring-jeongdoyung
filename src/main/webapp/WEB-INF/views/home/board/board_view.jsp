...
$(document).ready(function() {
	$("#insertReplyBtn").on("click", function() {//댓글등록버튼을 클릭했을 때 구현내용(아래)
		if("${session_enabled}" == "") {//버튼클릭시 비로그인상태면 로그인화면으로 유도
		...
		$.ajax({//통신프로그램 : J쿼리에서 내장된 함수 ajax({}); 비동기통신(HTTP동기통신 - 웹페이지의 단점을 해소 Ajax)
		//최초로 상용화 적용되었던 곳이 파일 업로드/다운로드에 Ajax기능이 적용.
		//서버(RestAPI서버 = 컨트롤러)-클라이언트(PC브라우저 ajax = jsp단)
			...
			success:function(result) {//응답이 성공하면(상태값200)위경로에서 반환받은 result(json데이터)를 이용해서 화면을 재구현
				...
				$("#reply_count").text(parseInt(reply_count)+1);//셋Set
				$("#reply_page").val("1");//댓글 확인이 바로 되도록 1페이지값으로 Set
