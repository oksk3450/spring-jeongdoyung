...
<input type="hidden" id="reply_page" value="1"><!-- #btn_reply_list 클릭할때 가져올 페이지값 -->
...
<script>
/* 위 댓글페이징에서 링크 태그의 페이지 이동을 방지하고, btn_reply_list버튼을 클릭해서
   /reply/reply_list/${boardVO.bno}/{1} -> 링크한 페이지값으로 대체해서 실행하는 코드(아래)
*/
$(document).ready(function(){
	$(".pageVO").on("click", "li a", function(event){
		event.preventDefault();//a태그의 기본기능인 이동기능을 막음.
...
$(document).ready(function(){
	$("#updateReplyBtn").on("click", function(){
		...
		$.ajax({
			...
			success:function(result){
				if(result=="success") {//= 대입, == 비교, 1==='1' 타입포함 비교
					alert("수정에 성공하였습니다.");
...
$(document).ready(function() {
	$("#insertReplyBtn").on("click", function() {//댓글등록버튼을 클릭했을 때 구현내용(아래)
		...
		if(reply_text == "" || replyer == ""){
			// boolean타입 1+0=1, 1+1+1, 0+1=1 (|| = or(둘중에 하나라도 true라면 모두 true)
			// boolean타입 1x0=0, 1x1=1, 0x1=0 (&& = and 두개다 true여야 모두 true)
			alert("댓글 내용과 댓글작성자 입력은 필수 입니다.");
			return false;
		}
		$.ajax({//통신프로그램
			...
			success:function(result) {//응답이 성공하면(상태값200)위경로에서 반환받은 result(json데이터)를 이용해서 화면을 재구현
				...
				*/
				...
				//입력이 success된 후에 페이지는 댓글갯수 1증가 + 1페이지로 가고, replyList()댓글 목록 호출 
				var reply_count = $("#reply_count").text();//$("영역").val(input데이터),
				if(reply_count == ""){reply_count=0;}//오라클 초기값이 null일때 체크
				$("#reply_count").text(parseInt(reply_count)+1);//$("영역").text(영역안쪽의 문자열)
				$("#reply_page").val(1);
				replyList();
				$("#replyer").val("");
