              ...
              <!-- /.card-header -->
                ...
                <div class="card-body">
                  <div class="form-group">
                    ...
                    <input type="text" value="" class="form-control" name="board_type" id="board_type" placeholder="게시판 타입을 영문으로 입력해주세요" required>
                  </div>
                  <div class="form-group">
...
<!-- 현재 J쿼리코어파일 footer.jsp에 있기때문에 아래 j쿼리 실행은 코어 아래쪽에 있어야 함. -->
<!-- 이렇게 상단부터 프로그램 내용을 컴파일하는 과정을 인터프리터형 프로그램 언어라 함 -->
<script>
jQuery(document).ready(function(){
	jQuery("#board_type").bind("blur",function(){
		if(jQuery(this).val() != "") {
			...
			if(jQuery.inArray(jQuery(this).val(),board_type_array) >= 0) {
				//indexOf함수는 반환값이 인덱스값을 반환. 0보다 크다면 인덱스가 존재
				alert("기존 게시판이 존재합니다. 다시 입력해 주세요");
				jQuery("#btn_insert").attr("disabled",true);	
			}else{
				alert("중복된 게시판이 존재하지 않습니다.");
